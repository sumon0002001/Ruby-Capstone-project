...
class ErrorChecker
  ...
  def end_error
    keyw_count = 0
    end_count = 0
    @checker.contents.each_with_index do |str_val, _index|
  ...
  def empty_line_error
    @checker.contents.each_with_index do |str_val, indx|
  ...
  def check_class_empty_line(str_val, indx)
    ...
    log_error("line:#{indx + 2} #{msg}") if @checker.contents[indx + 1].strip.empty?
  end
  ...
  def check_def_empty_line(str_val, indx)
    ...
    log_error("line:#{indx + 2} #{msg1}") if @checker.contents[indx + 1].strip.empty?
    log_error("line:#{indx + 1} #{msg2}") if @checker.contents[indx - 1].strip.split(' ').first.eql?('end')
  end

  def check_end_empty_line(str_val, indx)
    return unless str_val.strip.split(' ').first.eql?('end')

    log_error("line:#{indx} Extra empty line detected at block body end") if @checker.contents[indx - 1].strip.empty?
  end
